...
class AMQPBackend {
	...
	authHandler(model, tokenId, modelId, method, ctx, callback) {
		if (this.app.isAuthEnabled)
		{
			ctx = ctx || {};
			//FIXME: use this.app.registry.getModelByType('AccessToken')
			this.app.models.AccessToken.findById(tokenId, function(err, token) {
				if (err || token == null)
				{
					console.log("no matching token found", err);
					callback(err, false);
				}
				else
				{
